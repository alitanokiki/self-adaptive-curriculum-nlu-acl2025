...
def probabilistic_sampling(sorted_indices, probabilities, batch_size, seed):
    ...
    while len(indices) > 0:
        if len(indices) < batch_size:
            ...
            batch_indices = rng.choice(indices, size=batch_size, replace=False, p=probabilities)
...
sampled_indices = probabilistic_sampling(sorted_indices, probabilities, batch_size, seed)
sampled_train_dataset = new_train_dataset.select(sampled_indices)
