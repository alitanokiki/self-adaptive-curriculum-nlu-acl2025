...
def tokenize_and_add_prompt(examples):
    ...
    prompt_sentences = [sentence + f", this was a {mask_token} movie." for sentence in examples['tweet']]
...
sorted_train_dataset = new_train_dataset.select(sorted_indices[:64])

train_loader = DataLoader(sorted_train_dataset, batch_size=16, shuffle=False, collate_fn=collate_fn)
