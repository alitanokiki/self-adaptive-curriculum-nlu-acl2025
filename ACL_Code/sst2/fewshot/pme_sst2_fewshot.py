...
data = 'sst2'
strategie = 'pme_cut'
method = f'1e5_5_16_{strategie}__{args.model_type}_seed{args.seed}'
...
#def tokenize_and_add_prompt(examples):
 #   prompt_sentences = [sentence + ", this was a [MASK] movie." for sentence in examples['sentence']]
 #   return tokenizer(prompt_sentences, padding=True, truncation=True, max_length=128, return_tensors='pt')
...
sorted_indices = [x[0] for x in scores]

x = len(sorted_indices)
probabilities = np.array([i**2 for i in range(1, x + 1)])
