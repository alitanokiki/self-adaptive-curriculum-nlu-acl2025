...
train_sentiment_scores.sort(key=lambda x: x[1], reverse=True)
...
def probabilistic_sampling(sorted_indices, probabilities, batch_size, proportions, seed):
    ...
    with tqdm(total=len(sorted_indices), desc="Sampling progress") as pbar:
        while len(indices) > 0:
            num_samples1 = math.ceil(proportions[0] * batch_size)
            num_samples2 = math.ceil(proportions[1] * batch_size)
            ...
            if len(indices) < num_samples1:
                ...
                batch_indices1 = indices[torch.multinomial(probabilities, num_samples=num_samples1, replacement=False, generator=rng)]

            sampled_indices.extend(batch_indices1.tolist())
            indices = indices[~torch.isin(indices, batch_indices1)]
            probabilities = probabilities[:len(indices)].clone().detach()
            probabilities = probabilities / probabilities.sum()

        
            probabilities_reverse = probabilities.flip(0)
            if len(indices) < num_samples2:
                batch_indices2 = indices
            else:
                batch_indices2 = indices[torch.multinomial(probabilities_reverse, num_samples=num_samples2, replacement=False, generator=rng)]

            sampled_indices.extend(batch_indices2.tolist())
            indices = indices[~torch.isin(indices, batch_indices2)]
            probabilities = probabilities[:len(indices)].clone().detach()
            probabilities = probabilities / probabilities.sum()
...
sampled_train_dataset = train_dataset.select(sampled_indices[:64])
